...
def test_get_completed_starter_tasks():
    response = client.get('/task', params={"completed": True})
    assert response.status_code == 200
    assert response.json() == {
        "44c0c224-6084-48d0-876b-43f30f157014": {
            "description": "Buy food",
...
def test_get_not_completed_starter_tasks():
    response = client.get('/task', params={"completed": False})
    assert response.status_code == 200
    assert response.json() == {
        "953c3c2a-478b-48d7-9631-7b3113a1c4cc": {
            "description": "Finish exercise",
